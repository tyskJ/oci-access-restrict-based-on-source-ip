...
=====================================================================
OCI へのアクセス を送信元IPで制限する
=====================================================================
* `詳細 <https://qiita.com/tyskJ/items/0d8b2fe9cb52b53852ee>`_
...
.. code-block:: bash
  ...
  namespace = "${NAMESPACE}"
  key = "oci-access-restrict-based-on-source-ip/terraform.tfstate"
  auth = "SecurityToken"
...
.. note::
  ...
  * デプロイ後、 `詳細 <https://qiita.com/tyskJ/items/0d8b2fe9cb52b53852ee>`_ を参考に、 *Sign-on policy* を手動で修正してください

後片付け - ローカル -
=====================================================================
1. 環境削除
---------------------------------------------------------------------
* Network perimeter は本検証時に、Sign-on policy内ルールで利用するように手動変更している
* ルールで参照しているため、terraform destroy しようとすると、BadErrorResponseとなる
* その場合、destroy 実行前にstateから削除する
...
.. code-block:: bash
  ...
  COMPARTMENT_NAME="oci-access-restrict-based-on-source-ip"
